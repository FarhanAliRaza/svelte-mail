	...
	for (const style of styles) {
		...
		for (const decl of declarations) {
			...
			if (!EMAIL_SAFE_PROPERTIES.has(key)) {
				...
				if (
					key !== decl.substring(0, colonIndex).trim() ||
					!EMAIL_SAFE_PROPERTIES.has(decl.substring(0, colonIndex).trim())
				) {
					console.warn(
						`[styleToString] Unsupported or unsafe CSS property: "${key}" will be ignored.`
					);
					continue;
				}
			}
