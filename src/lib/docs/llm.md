...
<!-- All individual -->
<Button paddingTop="10px" paddingRight="20px" paddingBottom="10px" paddingLeft="20px">Explicit sides</Button>
```
...
**Note on Centering:** Because the `Img` component defaults to `display: block;`, using `text-align: center;` on a parent element (like `<Column>`) will *not* center the image. To center an image, apply `margin: 0 auto;` directly to the `Img` component's `style` prop:
...
<Link
  href="#"
  style="display: block; width: 220px; margin-left: auto; margin-right: 0;"
>
  Track Package
</Link>
```
